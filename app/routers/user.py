...
async def update_user(user_id: int, updated_user: UpdateUser, db: Annotated[Session, Depends(get_db)]):
    ...
    if user:
        ...
        db.commit()
        return {'status_code': status.HTTP_200_OK, 'transaction': 'Данные пользователя обновлены'}
    raise HTTPException(
        status_code=status.HTTP_404_NOT_FOUND,
        detail='Пользователь не найден')
...
async def delete_user(user_id: int, db: Annotated[Session, Depends(get_db)]):
    ...
    if user:
        ...
        db.commit()
        return {'status_code': status.HTTP_200_OK, 'transaction': 'Пользователь был удалён'}
    raise HTTPException(
        status_code=status.HTTP_404_NOT_FOUND,
        detail='Пользователь не найден')
